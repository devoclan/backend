...
| M8 | Full indexer coverage (all events, all contracts, already true as of M6 — see `EVENT_INDEXER.md`'s "Current Scope"), security review pass, observability dashboards, load test pass | M7 | Indexer coverage done; security review/observability/load test not started — none of that is per-module work |
