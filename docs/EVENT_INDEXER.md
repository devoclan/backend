...
`notifications` is the sixth subscriber (`src/modules/notifications/infrastructure/event-subscription.ts`/`dispatch-notifications-from-event.ts`), and the first one that isn't syncing a read model — it turns a handful of events into `Notification` rows instead. Deliberately narrow scope: only events whose own topic/payload names an actor address directly are handled (`delivery.driver_assigned`, `escrow.delivery_disputed`, `dispute-resolution.dispute_raised`, all four `identity-reputation` events, and five of `fleet`'s events) — sparser events (`delivery_created`, `escrow_funded`, `dispute_resolved_*`, ...) would need either a supplementary contract read or reaching into another module's read-model table to learn who to notify, and this module does neither (see `dispatch-notifications-from-event.ts`'s header comment). An address with no linked local account is silently skipped, not an error.
